def restore_gravity_assist noun, verb
...
catch :found_output do
  noun.each do |n|  
    verb.each do |v|
      memory = restore_gravity_assist n, v
      memory = parse_memory memory
      puts "Memory is now #{memory}"
      first_output = memory[0]
      result_noun = n
      result_verb = v
      throw :found_output if first_output == 19690720
    end
  end
end

puts "Noun and Verb values to reproduce the output 19690720 are respectively #{result_noun} and #{result_verb}"